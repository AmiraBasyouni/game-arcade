...
window.addEventListener("click", catch_click);
...
function draw_pillar_obstacle(gap_y_position, gap_size){
	if(gap_size < MIN_GAP_SIZE){
		alert("pillar height is insufficient, please increase gap size in game2.js");
	}
	...
	ctx.fillRect(pillar_obstacle.x, pillar_obstacle.y, pillar_obstacle.width, gap_y_position); // top pillar
	ctx.fillRect(pillar_obstacle.x, gap_y_position + gap_size, pillar_obstacle.width, canvasElement.height - gap_y_position + gap_size); // bottom pillar
}
...
function game_over_message(){
	...
	ctx.font = "32px serif";
	ctx.fillStyle = "white";
	ctx.fillText("click on canvas to restart", canvasElement.width/2, canvasElement.height/2 + 35);
	ctx.strokeStyle = "#454545";
	ctx.strokeText("click on canvas to restart", canvasElement.width/2, canvasElement.height/2 + 35);
